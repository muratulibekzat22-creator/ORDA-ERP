...
const documentInclude = {
  order: { select: { id: true, number: true, address: true, client: { select: { id: true, name: true } } } },
...
export async function getDocuments(actor: DocumentActor, filters: { orderId?: number; type?: DocumentType } = {}) {
  const ownerPartnerId = await partnerId(actor);
  return prisma.document.findMany({
    where: {
      ...(filters.orderId ? { orderId: filters.orderId } : {}),
      ...(filters.type ? { type: filters.type } : {}),
      ...(ownerPartnerId === undefined ? {} : { order: { partnerId: ownerPartnerId } }),
    },
...
export async function createDocument(input: { orderId: number; type: DocumentType; number: string; documentDate: Date; idempotencyKey: string; requestHash: string; actor: DocumentActor }) {
  ...
  try {
    const document = await prisma.document.create({
      data: { orderId: input.orderId, type: input.type, number: input.number, documentDate: input.documentDate, idempotencyKey: input.idempotencyKey, requestHash: input.requestHash },
      include: documentInclude,
    });
...
export async function deleteDocument(id: number, actor: DocumentActor) {
  ...
  const document = await prisma.document.findUnique({ where: { id }, select: { id: true, attachments: { select: { pathname: true } } } });
  if (!document) return null;
  if (document.attachments.length) await del(document.attachments.map((attachment) => attachment.pathname));
...
export async function getDocumentOrder(id: number, actor: DocumentActor) {
  ...
  if (actor.role === Role.PARTNER) {
    const ownerPartnerId = await partnerId(actor);
    const owned = await prisma.order.findFirst({ where: { id, partnerId: ownerPartnerId }, select: { id: true } });
