...
async function actorPartnerId(actor: AttachmentActor) {
  ...
  return (await prisma.partner.findUnique({ where: { userId: actor.userId }, select: { id: true } }))?.id ?? -1;
}
...
export async function listAttachments(orderId: number, actor: AttachmentActor) {
  ...
  return prisma.attachment.findMany({ where: { orderId }, select: publicSelect, orderBy: { createdAt: "desc" } });
}
...
export async function uploadAttachment(input: { orderId: number; documentId?: number; file: File; idempotencyKey: string; actor: AttachmentActor }) {
  if (input.actor.role !== Role.DIRECTOR && input.actor.role !== Role.MANAGER) throw new Error("FORBIDDEN");
  const fileName = safeFileName(input.file.name);
  const bytes = Buffer.from(await input.file.arrayBuffer());
  if (!validFileContent(fileName, input.file.type, bytes)) throw new Error("INVALID_FILE_TYPE");
  const requestHash = createHash("sha256").update(JSON.stringify({ orderId: input.orderId, documentId: input.documentId ?? null, fileName, contentType: input.file.type, size: bytes.byteLength })).update(bytes).digest("hex");
  const repeated = await prisma.attachment.findUnique({ where: { idempotencyKey: input.idempotencyKey }, select: { ...publicSelect, requestHash: true } });
  if (repeated) {
    if (!compareRequestHash(repeated.requestHash, requestHash)) throw new Error("IDEMPOTENCY_CONFLICT");
  ...
  try {
    const attachment = await prisma.attachment.create({ data: { orderId: input.orderId, documentId: input.documentId, uploadedById: input.actor.userId, fileName, pathname: blob.pathname, contentType: input.file.type, size: bytes.byteLength, idempotencyKey: input.idempotencyKey, requestHash }, select: publicSelect });
...
export async function getAttachmentContent(id: number, actor: AttachmentActor) {
  const attachment = await prisma.attachment.findUnique({ where: { id }, select: { id: true, orderId: true, pathname: true, fileName: true, contentType: true, size: true } });
  if (!attachment || !await canReadOrderAttachments(attachment.orderId, actor)) return null;
