...
export async function canAccessOrder360(
  orderId: number,
  actor: Order360Actor,
  ...
  if (actor.role === Role.PRODUCTION)
    return order.productions.some((row) => row.masterUserId === actor.userId);
  if (actor.role === Role.INSTALLER)
    return (
      order.installation?.installerUserId === actor.userId ||
      order.productions.some((row) => row.masterUserId === actor.userId)
    );
  if (actor.role === Role.MEASURER)
    return order.measurements.some(
      (row) => row.measurerUserId === actor.userId,
    );
  return false;
}

async function assertAccess(orderId: number, actor: Order360Actor) {
  if (!(await canAccessOrder360(orderId, actor)))
    throw new Order360Error("NOT_FOUND");
}
...
export async function evaluateGate(
  ...
  const order = await client.order.findUnique({
    ...
    include: {
      ...
      productions: {
        select: { percent: true, completedAt: true, actualEndAt: true },
      },
      blockers: {
        where: {
  ...
  if (target === OrderLifecycle.READY_FOR_INSTALLATION) {
    const productionDone = order.productions.some(
      (p) => !!p.completedAt || !!p.actualEndAt || p.percent >= 100,
    );
    checks = [
      {
        code: "PRODUCTION_COMPLETE",
        passed: productionDone,
  ...
  if (target === OrderLifecycle.INSTALLATION)
    checks = [
      {
        code: "INSTALLATION_DATE",
        passed: !!order.installation?.scheduledAt,
        message: "Дата монтажа не назначена",
      },
      {
        code: "INSTALLER",
        passed: !!order.installation?.installerUserId,
        message: "Монтажник не назначен",
      },
      {
        code: "INSTALLATION_PACKAGE",
        passed: order.installation?.packageConfirmed === true,
        message: "Монтажный пакет не подтверждён",
      },
      {
        code: "ADDRESS",
        passed: order.address.trim().length > 0,
        message: "Адрес не указан",
      },
      noCritical,
    ];
  if (target === OrderLifecycle.ACCEPTANCE)
    checks = [
      {
        code: "INSTALLATION_COMPLETE",
        passed:
          !!order.installation?.completedAt || order.installationCompleted,
        message: "Монтаж не завершён",
      },
      noCritical,
    ];
...
export async function availableTransitions(
  ...
  return {
    ...
    transitions: await Promise.all(
      candidates.map(async (to) => ({
        to,
        gate: await evaluateGate(orderId, to),
      })),
    ),
  };
}
    ...
    async (tx) => {
      ...
      if (existing) {
        if (!compareRequestHash(existing.requestHash, input.requestHash))
          throw new Order360Error("IDEMPOTENCY_CONFLICT");
      ...
      if (!gate.passed) {
        ...
        await tx.orderGateOverride.create({
          data: {
            ...
            snapshot: gate as unknown as Prisma.InputJsonValue,
            authorId: actor.userId,
            idempotencyKey: `${input.key}:override`,
...
export async function openBlocker(
  ...
  const existing = await prisma.orderBlocker.findUnique({
    where: { idempotencyKey: input.key },
  });
  if (existing) {
    if (!compareRequestHash(existing.requestHash, input.requestHash))
...
export async function resolveBlocker(
  ...
  const blocker = await prisma.orderBlocker.findUnique({
  ...
  return prisma.$transaction(async (tx) => {
    const updated = await tx.orderBlocker.update({
      where: { id: blocker.id },
      data: {
...
export async function confirmMilestone(
  ...
  const fields: Record<string, Prisma.OrderUpdateInput> = {
    ...
    "set-production-deadline": {
      productionDeadline: new Date(String(input.value)),
    },
...
export async function assignInstallation(
  input: {
    orderId: number;
    scheduledAt: Date;
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (existing) return { event: existing, created: false };
    const installer = await tx.user.findFirst({
      where: { id: input.installerUserId, role: Role.INSTALLER, active: true },
    });
    ...
    if (updated.count !== 1) throw new Order360Error("STALE_VERSION");
    await tx.orderInstallation.upsert({
      where: { orderId: input.orderId },
      create: {
        orderId: input.orderId,
        scheduledAt: input.scheduledAt,
        installerUserId: input.installerUserId,
        packageConfirmed: input.packageConfirmed,
        comment: input.comment,
      },
      update: {
        scheduledAt: input.scheduledAt,
...
export async function completeInstallation(
  ...
  await assertAccess(orderId, actor);
  if (!(actor.role === Role.DIRECTOR || actor.role === Role.INSTALLER))
    throw new Order360Error("FORBIDDEN");
  return prisma.$transaction(async (tx) => {
    const existing = await tx.orderLifecycleEvent.findUnique({
    ...
    const updated = await tx.order.updateMany({
      ...
      data: { installationCompleted: true, version: { increment: 1 } },
    });
    if (updated.count !== 1) throw new Order360Error("STALE_VERSION");
    await tx.orderInstallation.update({
      where: { orderId },
...
export async function orderAttention(orderId: number, actor: Order360Actor) {
  ...
  const order = await prisma.order.findUniqueOrThrow({
    ...
    include: {
      documents: { select: { type: true } },
      measurements: { select: { id: true } },
      productions: { select: { plannedEndAt: true, completedAt: true } },
      blockers: {
        where: { status: OrderBlockerStatus.OPEN },
  ...
  if (
    ...
    order.promisedAt &&
    order.promisedAt < now
  )
    push("PAYMENT_OVERDUE", "WARNING", "Просрочен платёж", order.promisedAt);
  if (
    !order.contractConfirmedAt &&
    !order.documents.some((d) => d.type === "CONTRACT")
  )
  ...
  if (
    ...
    push("INSTALLATION_NOT_SCHEDULED", "WARNING", "Монтаж не назначен");
  if (
    (actor.role === Role.DIRECTOR ||
      actor.role === Role.ACCOUNTANT ||
  ...
  for (const blocker of order.blockers)
    signals.push({
      ...
      responsible: blocker.responsibleUser,
      dueAt: blocker.dueAt,
...
export async function orderOverview(orderId: number, actor: Order360Actor) {
  ...
  const days = order.promisedAt
    ? Math.ceil((order.promisedAt.getTime() - Date.now()) / 86400000)
    : null;
  const result: Record<string, unknown> = {
    ...
    health: attention.some((x) => x.severity === "CRITICAL")
      ? "CRITICAL"
      : attention.length
        ? "ATTENTION"
        : "OK",
...
export async function orderTimeline(
  ...
  const safePage = Math.max(1, page),
    take = safePage * safeSize;
  const includeLegacy =
    actor.role === Role.DIRECTOR ||
    actor.role === Role.ACCOUNTANT ||
    actor.role === Role.MANAGER;
  const [typed, statuses, legacy] = await Promise.all([
    prisma.orderLifecycleEvent.findMany({
      where: { orderId },
      orderBy: { createdAt: "desc" },
      take,
      select: {
...
export async function orderDocuments(
  ...
  return prisma.document.findMany({
    ...
    orderBy: { createdAt: "desc" },
    skip: (Math.max(1, page) - 1) * safeSize,
    take: safeSize,
  });
}
