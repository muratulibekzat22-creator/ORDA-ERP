...
export async function getAnalytics(filters: Filters) {
  ...
  const where: Prisma.ClientWhereInput = {
    createdAt: { gte: start, lte: end }, active: true, deletedAt: null,
  ...
  const leads = await prisma.client.findMany({ where, include: {
    ...
    leadConversion: { select: { createdAt: true, orderId: true, order: { select: { deletedAt: true } } } },
  ...
  const hasActiveOrder = (lead: typeof leads[number]) => Boolean(lead.leadConversion && !lead.leadConversion.order.deletedAt);
  const countStage = (stage: LeadStage) => leads.filter((lead) => reached(lead, stage) && (stage !== LeadStage.WON || hasActiveOrder(lead))).length;
  ...
  const managers = [...managerMap.values()].map((group) => {
    const has = (lead: typeof leads[number], stage: LeadStage) => reached(lead, stage), groupWon = group.leads.filter((lead) => has(lead, LeadStage.WON) && hasActiveOrder(lead)).length, groupQualified = group.leads.filter((lead) => has(lead, LeadStage.QUALIFIED)).length;
  ...
  const orders = filters.role === Role.DIRECTOR ? await prisma.order.findMany({ where: { deletedAt: null, createdAt: { gte: start, lte: end } }, include: { partner: { select: { id: true, name: true } } } }) : [];
  ...
  const months = Array.from({ length: 6 }, (_, index) => { const date = new Date(now.getFullYear(), now.getMonth() - 5 + index, 1), rows = leads.filter((lead) => lead.createdAt.getFullYear() === date.getFullYear() && lead.createdAt.getMonth() === date.getMonth()); return { month: date.toLocaleDateString("ru-RU", { month: "short" }), leads: rows.length, won: rows.filter((lead) => reached(lead, LeadStage.WON) && hasActiveOrder(lead)).length }; });
