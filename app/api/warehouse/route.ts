...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!only(body, ["action", "type", "materialId", "quantity", "price", "orderId", "supplier", "comment", "operationAt", "expiresAt", "reversalOfId"])) return NextResponse.json({ error: "Недопустимые поля" }, { status: 400 });
    ...
    const reversalOfId = body.reversalOfId == null || body.reversalOfId === "" ? undefined : positiveId(body.reversalOfId);
    ...
    if (!type || !materialId || orderId === null || reversalOfId === null || quantity === null || price === null || supplier === null || comment === null || (type === "return" && !reversalOfId) || (operationAt && (Number.isNaN(operationAt.getTime()) || operationAt.getTime() > Date.now() + 300_000)) || (expiresAt && (Number.isNaN(expiresAt.getTime()) || expiresAt.getTime() <= Date.now()))) return NextResponse.json({ error: "Некорректные данные операции" }, { status: 400 });
    const payload = { type, materialId, orderId, quantity, price, supplier, comment, reversalOfId, operationAt: operationAt?.toISOString(), expiresAt: expiresAt?.toISOString() };
    const result = await createWarehouseOperation({ data: { type, materialId, orderId, quantity, price, supplier, comment, operationAt, expiresAt, reversalOfId }, key: idempotency.key, requestHash: createRequestHash(payload), actor: actor(auth.session!) });
    return NextResponse.json(result.result, { status: result.replayed ? 200 : 201 });
