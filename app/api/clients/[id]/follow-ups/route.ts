...
export async function POST(request: Request, { params }: Context) {
  ...
  if (auth.session!.user.role !== Role.MANAGER && auth.session!.user.role !== Role.DIRECTOR && auth.session!.user.role !== Role.OPERATIONS_DIRECTOR) return NextResponse.json({ error: "Недостаточно прав" }, { status: 403 });
