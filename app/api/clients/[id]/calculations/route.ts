...
import { calculateStair, type CalculationLineInput, type StairMaterial, type StairRates } from "@/lib/calculator/stair-calculation";
...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    const managerMinimumTotal = equivalentSteps * Number(materialTariff?.managerMinimumPrice ?? materialTariff?.salePrice ?? 0) + lines.reduce((sum, line) => { const tariff = line.code ? byCode.get(line.code) : undefined; return sum + (line.enabled === false ? 0 : line.quantity * Number(tariff?.managerMinimumPrice ?? tariff?.salePrice ?? line.unitSale)); }, 0);
