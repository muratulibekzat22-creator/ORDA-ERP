...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    const pricingMode = body.pricingMode === "MANUAL" ? "MANUAL" : "CALCULATOR";
    const manualPricingComment = typeof body.manualPricingComment === "string"
      ? body.manualPricingComment.trim().slice(0, 1000)
      : "";
    if (pricingMode === "MANUAL" && !manualPricingComment)
      return NextResponse.json({ error: "Укажите основание ручной цены" }, { status: 400 });
    const finalPrice = body.finalPrice ?? body.clientPrice;
    if (pricingMode === "MANUAL" && (!Number.isFinite(Number(finalPrice)) || Number(finalPrice) <= 0))
      return NextResponse.json({ error: "Укажите финальную цену варианта" }, { status: 400 });
    ...
    const calculatedPrice = calculated.baseClientPrice + calculated.lines.reduce((sum, line) => sum + line.totalSale, 0) + calculated.deliveryCharge;
    const snapshot = JSON.parse(JSON.stringify({
      ...calculated,
      pricingMode,
      calculatedPrice,
      finalPrice: calculated.clientPrice,
      discountOrMarkup: calculated.clientPrice - calculatedPrice,
      manualPricingComment: manualPricingComment || null,
    }));
