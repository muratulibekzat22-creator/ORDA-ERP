...
export async function POST(_: Request, { params }: Context) {
  ...
  } catch (error) { if (error instanceof Error && error.message === "PROPOSAL_NOT_ACCEPTED") return NextResponse.json({ error: "Сначала отметьте КП как принятое" }, { status: 409 }); if (error instanceof Error && error.message === "LEAD_NOT_WON") return NextResponse.json({ error: "Сначала переведите заявку в WON" }, { status: 409 }); if (error instanceof Error && error.message === "LEAD_NOT_FOUND") return NextResponse.json({ error: "Заявка не найдена" }, { status: 404 }); return NextResponse.json({ error: "Не удалось оформить заказ" }, { status: 409 }); }
}
