...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      const passwordChanged = typeof body.password === "string" && Boolean(body.password);
      const accessChanged = (typeof body.active === "boolean" && body.active !== user.active) || (role !== undefined && role !== user.role);
      return tx.user.update({ where: { id }, data: { ...(typeof body.name === "string" && body.name.trim() ? { name: body.name.trim() } : {}), ...(typeof body.phone === "string" ? { phone: body.phone.trim() || null } : {}), ...(typeof body.active === "boolean" ? { active: body.active } : {}), ...(role ? { role } : {}), ...(passwordChanged ? { password: await bcrypt.hash(body.password as string, 12), passwordChangedAt: new Date(), mustChangePassword: true, failedLoginAttempts: 0, lockedUntil: null } : {}), ...(passwordChanged || accessChanged ? { sessionVersion: { increment: 1 } } : {}) }, select });
