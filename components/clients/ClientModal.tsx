...
const initial: ClientDraft = { name: "", phone: "+7", whatsapp: "+7", city: "Алматы", address: "", source: "WhatsApp", manager: "", status: "Новое обращение", estimateNotes: "", estimatedAmount: "0" };
...
export default function ClientModal({ open, onClose, onSave, saving = false }: Props) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setFieldError(""); if (form.phone.replace(/\D/g, "").length < 11) return setFieldError("Укажите корректный телефон"); if (!form.estimateNotes.trim()) return setFieldError("Кратко опишите запрос"); try { await onSave(form); setForm(initial); } catch { /* Parent keeps the form open and displays the API error. */ } }
