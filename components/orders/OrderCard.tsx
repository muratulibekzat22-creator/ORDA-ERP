...
export default function OrderCard({ order }: Props) {
  ...
  return <div className="rounded-2xl border border-slate-700 bg-[#101827] p-6">
    <div className="flex items-center justify-between"><div><h2 className="text-2xl font-bold text-white">Заказ {order.number}</h2><p className="mt-2 text-slate-400">{order.client?.name}</p>{order.partner?.name && <p className="mt-1 text-sm text-cyan-400">Цех: {order.partner.name}</p>}</div><div className="min-w-52"><select aria-label="Статус заказа" value={status} disabled={isSavingStatus} onChange={(event) => changeStatus(event.target.value)} className={`w-full rounded-xl px-4 py-2 font-medium outline-none transition disabled:cursor-wait disabled:opacity-70 ${statusColor(status)}`}>{!statuses.includes(status) && <option value={status}>{status}</option>}{statuses.map((item) => <option key={item} value={item}>{item}</option>)}</select>{statusError && <p className="mt-2 text-sm text-red-400">{statusError}</p>}</div></div>
