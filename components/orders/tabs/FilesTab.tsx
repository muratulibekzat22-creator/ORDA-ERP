  ...
  return (
    <div className="space-y-5">
      {canManage && (
        <form
          onSubmit={upload}
          className="rounded-2xl border border-dashed border-slate-600 bg-[#101827] p-6"
        >
          <h2 className="text-xl font-bold text-white">Загрузка файла</h2>
          <p className="mt-2 text-sm text-slate-400">
            PDF, изображения, Word и Excel. Максимальный размер — 10 МБ. Файлы
            хранятся в закрытом хранилище.
          </p>
          <div className="mt-4 flex flex-wrap gap-3">
            <input
              ...
              className="min-w-64 flex-1 rounded-xl border border-slate-700 bg-slate-900 p-3 text-slate-300 file:mr-4 file:rounded-lg file:border-0 file:bg-blue-600 file:px-3 file:py-2 file:text-white"
            />
            <button
      ...
      <div className="rounded-2xl border border-slate-700 bg-[#101827]">
        <div className="border-b border-slate-700 p-5">
          <h2 className="text-xl font-bold text-white">Файлы заказа</h2>
        </div>
        {loading ? (
          <p className="p-6 text-slate-400">Загрузка…</p>
        ) : !files.length ? (
          <p className="p-6 text-slate-400">Файлы ещё не загружены</p>
        ) : (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs text-slate-400">
                      ...
                      {item.uploadedBy?.name ?? "Удалённый пользователь"} ·{" "}
                  ...
                  <div className="flex gap-2">
                    {canPreview && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        href={`/api/attachments/${item.id}?disposition=inline`}
                    ...
                    <a
                      ...
                      className="rounded-lg bg-blue-700 p-2 text-white hover:bg-blue-600"
                    >
                      <Download size={18} />
                    </a>
                    {canManage && (
                      <button
                        ...
                        className="rounded-lg bg-red-900 p-2 text-white hover:bg-red-800 disabled:opacity-50"
                      >
                        <Trash2 size={18} />
                      </button>
                    )}
