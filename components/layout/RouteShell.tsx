...
const sections = [
  { title: "Главное", items: [["/", "Главная", LayoutDashboard], ["/operations", "Операционное управление", ShieldCheck]] },
...
export default function RouteShell({
  ...
  const visible = (href: string) => {
    ...
    if (href === "/partner-management") return role === "DIRECTOR" || (role === "OPERATIONS_DIRECTOR" && session?.user.companyOperationsEnabled === true);
    if (href === "/marketing") return role === "DIRECTOR" || (role === "OPERATIONS_DIRECTOR" && session?.user.companyOperationsEnabled === true) || role === "MARKETER";
    if (role === "OPERATIONS_DIRECTOR")
      return href === "/operations" || (session?.user.companyOperationsEnabled === true && ["/clients", "/orders", "/measurements", "/marketing", "/calendar", "/production", "/warehouse", "/partner-management", "/reports", "/documents"].includes(href));
    if (role === "MEASURER")
    ...
    if (role === "DIRECTOR")
      return ["/", "/operations", "/clients", "/orders", "/measurements", "/marketing", "/calendar", "/production", "/warehouse", "/employees", "/payroll", "/finance", "/partner-management", "/reports", "/documents", "/settings"].includes(href);
