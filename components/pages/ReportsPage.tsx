...
function ReportContent({ report }: { report: ReportsReadModel }) {
  ...
  return <>
    ...
    <Panel title="Продажи и платежи"><div className="grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-7"><Stat label="Заказов" value={report.sales.count}/><Stat label="Без цены производства" value={report.dataQuality.missingProductionPrice}/><Stat label="Средний чек" value={money(report.sales.averageOrder)}/><Stat label="Завершено" value={report.sales.completed}/><Stat label="Получено" value={money(report.payments.received)}/><Stat label="Остаток" value={money(report.payments.remaining)}/>{report.sales.grossMargin !== undefined && <Stat label="Валовая маржа" value={report.sales.grossMargin === null ? "Недостаточно данных" : money(report.sales.grossMargin)}/>}</div></Panel>
    {report.finance && <Panel title="Финансовая сводка"><div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5"><Stat label="Продажи" value={money(report.finance.sales)}/><Stat label="Получено от клиентов" value={money(report.finance.customerReceived)}/><Stat label="К получению" value={money(report.finance.customerRemaining)}/><Stat label="Согласовано партнёрам" value={money(report.finance.partnerAgreed)}/><Stat label="Выплачено партнёрам" value={money(report.finance.partnerPaid)}/><Stat label="К выплате партнёрам" value={money(report.finance.partnerRemaining)}/><Stat label="Записанные расходы" value={money(report.finance.recordedExpenses)}/><Stat label="Валовая маржа" value={report.finance.grossMargin === null ? "Недостаточно данных" : money(report.finance.grossMargin)}/><Stat label="Чистая прибыль" value={report.finance.netProfit === null ? "Недостаточно данных" : money(report.finance.netProfit)}/><Stat label="Payroll начислено" value={money(report.finance.payrollAccrued)}/><Stat label="Payroll выплачено" value={money(report.finance.payrollPaid)}/><Stat label="Payroll к выплате" value={money(report.finance.payrollPayable)}/></div></Panel>}
    {report.dataQuality.tasks.length > 0 && <Panel title="Что нужно дополнить"><p className="mb-3 text-sm text-slate-400">Список сформирован автоматически по заказам выбранного месяца. Директор видит общий контроль, менеджер получает свои пункты на главной.</p><div className="space-y-2">{report.dataQuality.tasks.map(item=><a key={item.orderId} href={`/orders/${item.orderId}`} className="block rounded-xl border border-amber-500/20 bg-amber-500/5 p-3"><div className="flex flex-wrap justify-between gap-2"><strong className="text-white">{item.number} · {item.client}</strong><span className="text-sm text-slate-400">{item.manager}</span></div><p className="mt-2 text-sm text-amber-200">{item.missingFields.join(" · ")}</p></a>)}</div></Panel>}
    ...
    <Panel title="Последние заказы"><div className="hidden overflow-x-auto md:block"><table className="w-full text-sm"><thead className="text-left text-slate-500"><tr>{["№","Клиент","Менеджер","Сумма","Цена производства","Получено","Остаток","Статус"].map(x=><th key={x} className="px-3 py-2">{x}</th>)}</tr></thead><tbody>{report.orders.map(item=><tr key={item.id} className="border-t border-slate-800"><td className="px-3 py-3 font-semibold"><a className="text-blue-300 hover:text-blue-200" href={`/orders/${item.id}`}>{item.number}</a></td><td className="px-3">{item.client}</td><td className="px-3">{item.manager}</td><td className="px-3">{money(item.amount)}</td><td className={`px-3 ${item.productionPrice === null ? "font-semibold text-amber-300" : "text-cyan-300"}`}>{item.productionPrice === null ? "Не заполнена" : money(item.productionPrice)}</td><td className="px-3">{money(item.received)}</td><td className="px-3">{money(item.remaining)}</td><td className="px-3">{item.status}</td></tr>)}</tbody></table></div><div className="grid gap-3 md:hidden">{report.orders.map(item=><a href={`/orders/${item.id}`} key={item.id} className="rounded-xl bg-slate-950 p-4"><div className="flex justify-between gap-3"><strong className="text-blue-300">{item.number}</strong><span className="text-xs text-slate-400">{item.status}</span></div><p className="mt-1 text-sm text-white">{item.client}</p><div className="mt-3 grid grid-cols-2 gap-1 text-xs text-slate-400"><span>Сумма {money(item.amount)}</span><span className={item.productionPrice === null ? "text-amber-300" : ""}>Производство {item.productionPrice === null ? "не заполнено" : money(item.productionPrice)}</span><span>Остаток {money(item.remaining)}</span></div></a>)}</div></Panel>
