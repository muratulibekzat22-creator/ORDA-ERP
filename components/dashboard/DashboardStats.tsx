...
type Props = { totalClients: number; totalOrders: number; totalProduction: number; totalRevenue: number; totalProfit: number; totalDebt: number; totalPartnerBalance: number; showInternalFinance?: boolean };
const money = (value: number) => `${value.toLocaleString("ru-RU")} ₸`;

export default function DashboardStats({ totalClients, totalOrders, totalProduction, totalRevenue, totalProfit, totalDebt, totalPartnerBalance, showInternalFinance = false }: Props) {
  const stats = [
    ...
    { title: "Дебиторская задолженность", value: money(totalDebt), color: "text-orange-400", icon: Landmark },
    ...(showInternalFinance ? [
      { title: "Прибыль ALTYN SAPA", value: money(totalProfit), color: "text-emerald-400", icon: TrendingUp },
