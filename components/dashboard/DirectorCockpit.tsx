...
function SalesDashboard({ data }: { data: SalesPayload }) {
  ...
  return <>
    <div className="grid min-w-0 grid-cols-2 gap-3 md:grid-cols-4">
      {director ? <><Metric href="/clients" label="Заявки" value={m.newLeads} featured/><Metric href="/orders" label="Заказы" value={m.orders} featured/><Metric href="/orders" label="Продажи" value={money(m.totalSales)} featured/><Metric href="/finance" label="Получено" value={money(m.receivedPrepayment)} featured/><Metric href="/finance" label="Остаток клиентов" value={money(m.balanceToReceive)}/><Metric href="/partners" label="К выплате партнёрам" value={money(m.partnerBalancePayable ?? 0)}/><Metric href="/calendar" label="Задачи сегодня" value={m.tasksToday}/><Metric href="/calendar" label="Просрочено" value={m.overdueTasks + m.overdueNextActions + m.overdueOrders} alert/></>
