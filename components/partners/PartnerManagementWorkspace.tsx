...
type PartnerOrder = {
  ...
  order: { id: number; number: string; createdAt: string; client: { id: number; name: string; phone: string }; manager: { id: number | null; name: string }; address: string; staircase: string; material: string; amount: DecimalLike; companyProfit: DecimalLike; status: string; contract?: { number?: string | null } | null };
};
...
type Payload = {
  ...
  totals: { activePartners: number; orders: number; orderAmount: DecimalLike; received: DecimalLike; clientRemaining: DecimalLike; companyAmount: DecimalLike; partnerAccrued: DecimalLike; partnerPaid: DecimalLike; companyDebt: DecimalLike; partnerDebt: DecimalLike; averageOrder: DecimalLike; profit: DecimalLike };
...
const tabs: Array<[Tab, string]> = [["overview", "Обзор"], ["partners", "Партнёры"], ["orders", "Заказы партнёров"], ["settlements", "Взаиморасчёты"], ["operations", "Операции"], ["reports", "Отчёты"]];
...
export default function PartnerManagementWorkspace() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(payload);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Не удалось загрузить партнёров"); }
  ...
  const mutate = async (body: Record<string, unknown>, idempotent = false) => {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Операция не выполнена"); return false; }
  ...
  return <main className="min-w-0 space-y-5 overflow-x-hidden p-4 text-slate-100 sm:p-6 md:p-8">
    <header className="relative overflow-hidden rounded-[28px] border border-amber-300/20 bg-[#0b1220] p-5 sm:p-7">
      ...
      <div className="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div><p className="text-xs font-bold uppercase tracking-[0.25em] text-amber-300">Director workspace</p><h1 className="mt-2 text-3xl font-semibold text-white">Партнёры</h1><p className="mt-2 text-sm text-slate-400">Заказы, вознаграждения и взаиморасчёты в одном месте</p></div>
    ...
    <nav aria-label="Разделы партнёров" className="flex max-w-full gap-2 overflow-x-auto pb-1">{tabs.map(([value, label]) => <button key={value} type="button" onClick={() => setTab(value)} className={`min-h-11 shrink-0 rounded-xl px-4 text-sm font-semibold ${tab === value ? "bg-amber-300 text-slate-950" : "border border-slate-800 bg-slate-900 text-slate-300"}`}>{label}</button>)}</nav>
...
function Overview({ data }: { data: Payload }) {
  const cards: Array<[string, DecimalLike, "money" | "count"]> = [
    ["Активные партнёры", data.totals.activePartners, "count"], ["Заказы партнёров", data.totals.orders, "count"],
    ...
    ["Начислено партнёрам", data.totals.partnerAccrued, "money"], ["Выплачено партнёрам", data.totals.partnerPaid, "money"],
    ["Компания должна", data.totals.companyDebt, "money"], ["Партнёры должны", data.totals.partnerDebt, "money"],
...
function Partners({ data, busy, mutate }: { data: Payload; busy: boolean; mutate: (body: Record<string, unknown>, idem?: boolean) => Promise<boolean> }) {
  ...
  const [form, setForm] = useState({ name: "", kind: "REFERRER", phone: "", secondaryPhone: "", email: "", city: "", address: "", contactPerson: "", cooperationStartedAt: "", rewardRule: "ORDER_PERCENT", rewardPercent: "10", fixedAmount: "", comment: "" });
  ...
  return <div className="space-y-5"><div className="flex flex-col gap-3 sm:flex-row sm:justify-between"><div><h2 className="text-xl font-bold">Справочник партнёров</h2><p className="text-sm text-slate-400">Архивирование сохраняет всю историю.</p></div><button type="button" onClick={() => setOpen((value) => !value)} className={primary}><Plus size={17}/>Добавить партнёра</button></div>
    {open && <form onSubmit={submit} className={`${panel} grid gap-3 md:grid-cols-2 xl:grid-cols-3`}><FormTitle>Новый партнёр</FormTitle><Input label="Название / ФИО" value={form.name} required onChange={(value) => setForm({ ...form, name: value })}/><Select label="Тип" value={form.kind} onChange={(value) => setForm({ ...form, kind: value })} options={partnerKinds}/><Input label="Телефон" value={form.phone} onChange={(value) => setForm({ ...form, phone: value })}/><Input label="Дополнительный телефон" value={form.secondaryPhone} onChange={(value) => setForm({ ...form, secondaryPhone: value })}/><Input label="Email" type="email" value={form.email} onChange={(value) => setForm({ ...form, email: value })}/><Input label="Город" value={form.city} onChange={(value) => setForm({ ...form, city: value })}/><Input label="Адрес" value={form.address} onChange={(value) => setForm({ ...form, address: value })}/><Input label="Ответственное лицо" value={form.contactPerson} onChange={(value) => setForm({ ...form, contactPerson: value })}/><Input label="Дата начала" type="date" value={form.cooperationStartedAt} onChange={(value) => setForm({ ...form, cooperationStartedAt: value })}/><Select label="Вознаграждение" value={form.rewardRule} onChange={(value) => setForm({ ...form, rewardRule: value })} options={rewardRules}/>{form.rewardRule.includes("PERCENT") && <Input label="Процент" type="number" value={form.rewardPercent} onChange={(value) => setForm({ ...form, rewardPercent: value })}/>} {form.rewardRule === "FIXED" && <Input label="Фиксированная сумма" type="number" value={form.fixedAmount} onChange={(value) => setForm({ ...form, fixedAmount: value })}/>}<Input label="Комментарий" value={form.comment} onChange={(value) => setForm({ ...form, comment: value })}/><div className="flex items-end"><button disabled={busy} className={`${primary} w-full`}>Сохранить</button></div></form>}
...
function Orders({ data, busy, mutate, query, setQuery }: { data: Payload; busy: boolean; mutate: (body: Record<string, unknown>, idem?: boolean) => Promise<boolean>; query: string; setQuery: (value: string) => void }) {
  ...
  return <div className="space-y-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><label className="w-full max-w-xl text-sm text-slate-400">Поиск по партнёру, заказу, клиенту или телефону<div className="mt-1 flex gap-2"><input className={field} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Введите запрос"/><button type="button" className={secondary}><Search size={17}/></button></div></label><div className="flex flex-col gap-2 sm:flex-row"><button type="button" onClick={() => setMode("link")} className={secondary}><Link2 size={17}/>Привязать существующий</button><button type="button" onClick={() => setMode("create")} className={primary}><Plus size={17}/>Новый заказ партнёра</button></div></div>
    {mode === "link" && <form onSubmit={linkSubmit} className={`${panel} grid gap-3 md:grid-cols-2`}><FormTitle>Привязать существующий заказ</FormTitle><Select label="Партнёр" value={link.partnerId} onChange={(value) => setLink({ ...link, partnerId: value })} options={data.partners.filter((item) => item.businessStatus !== "ARCHIVED").map((item) => [String(item.id), item.name])}/><div><span className="text-sm text-slate-400">Поиск заказа</span><div className="mt-1 flex gap-2"><input className={field} value={search} onChange={(event) => setSearch(event.target.value)}/><button type="button" className={secondary} onClick={() => void findOrders()}><Search size={16}/></button></div></div><Select label="Заказ" value={link.orderId} onChange={(value) => setLink({ ...link, orderId: value })} options={results.map((item) => [String(item.id), `${item.number} · ${item.client.name} · ${money(item.amount)}${item.partnerRelation ? ` · связан с ${item.partnerRelation.partner.name}` : ""}`])}/><Select label="Правило (или настройка партнёра)" value={link.rewardRule} onChange={(value) => setLink({ ...link, rewardRule: value })} options={[["", "По умолчанию"], ...rewardRules]}/><Input label="Процент" type="number" value={link.rewardPercent} onChange={(value) => setLink({ ...link, rewardPercent: value })}/><Input label="Фиксированная сумма" type="number" value={link.fixedAmount} onChange={(value) => setLink({ ...link, fixedAmount: value })}/><Input label="Комментарий" value={link.comment} onChange={(value) => setLink({ ...link, comment: value })}/><div className="flex items-end gap-2"><button disabled={busy} className={primary}>Привязать</button><button type="button" onClick={() => setMode(null)} className={secondary}>Отмена</button></div></form>}
    {mode === "create" && <form onSubmit={createSubmit} className={`${panel} grid gap-3 md:grid-cols-2 xl:grid-cols-3`}>
      ...
      <Select label="Партнёр" value={order.partnerId} onChange={(value) => setOrder({ ...order, partnerId: value })} options={data.partners.filter((item) => item.businessStatus === "ACTIVE").map((item) => [String(item.id), item.name])}/>
...
function OrderCards({ orders, actions }: { orders: PartnerOrder[]; actions?: (item: PartnerOrder) => React.ReactNode }) {
  return <div className="grid gap-4">{orders.map((item) => <article key={item.id} className={panel}><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-bold text-white">{item.order.number} · {item.partner.name}</h3><p className="mt-1 text-sm text-slate-400">{item.order.client.name} · {item.order.client.phone} · {item.order.staircase}</p><p className="text-sm text-slate-500">{item.order.address}</p></div><Status value={item.settlementStatus}/></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-6"><Mini label="Заказ" value={money(item.metrics.orderAmount)}/><Mini label="Получено" value={money(item.metrics.received)}/><Mini label="Остаток клиента" value={money(item.metrics.clientRemaining)}/><Mini label="Сумма компании" value={money(item.metrics.companyAmount)}/><Mini label="Начислено партнёру" value={money(item.metrics.partnerAccrued)}/><Mini label="Баланс партнёра" value={money(item.metrics.partnerBalance)}/></div><details className="mt-4 rounded-xl bg-slate-950 p-3"><summary className="cursor-pointer font-semibold">Полные денежные показатели</summary><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3"><Mini label="Компания получила" value={money(item.metrics.companyClientReceived)}/><Mini label="Партнёр получил" value={money(item.metrics.clientPaidToPartner)}/><Mini label="Партнёр передал" value={money(item.metrics.partnerTransferred)}/><Mini label="Компания должна" value={money(item.metrics.companyDebt)}/><Mini label="Партнёр должен" value={money(item.metrics.partnerDebt)}/><Mini label="План партнёра" value={money(item.metrics.partnerPlanned)}/></div></details>{actions && <div className="mt-4 flex flex-wrap gap-2">{actions(item)}</div>}</article>)}{!orders.length && <Empty/>}</div>;
}
...
function Reports({ data }: { data: Payload }) {
  ...
  return <div className="space-y-5"><section className={`${panel} grid gap-3 md:grid-cols-2 xl:grid-cols-4`}><FormTitle>Выписка по партнёру</FormTitle><Select label="Партнёр" value={partnerId} onChange={setPartnerId} options={data.partners.map((item) => [String(item.id), item.name])}/><Input label="С" type="date" value={from} onChange={setFrom}/><Input label="По" type="date" value={to} onChange={setTo}/><div className="flex flex-col gap-2 sm:flex-row xl:col-span-4"><a className={primary} href={`/api/partner-management/${partnerId}/statement?format=pdf&${query}`}><Download size={17}/>PDF</a><a className={secondary} href={`/api/partner-management/${partnerId}/statement?format=csv&${query}`}><Download size={17}/>CSV</a></div></section><section className={panel}><h2 className="text-lg font-bold">Аналитика партнёров</h2><div className="mt-4 grid gap-3 md:grid-cols-2">{data.charts.partners.map((item) => <div key={item.partnerId} className="rounded-xl bg-slate-950 p-4"><p className="font-bold">{item.name}</p><p className="mt-2 text-sm text-slate-400">{item.orders} заказов · продажи {money(item.sales)} · прибыль {money(item.profit)} · баланс {money(item.debt)}</p></div>)}{!data.charts.partners.length && <Empty/>}</div></section></div>;
}
