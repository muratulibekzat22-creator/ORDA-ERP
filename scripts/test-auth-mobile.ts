...
const auth = read("app/api/auth/[...nextauth]/route.ts"), login = read("app/login/page.tsx"), schema = read("prisma/schema.prisma"), proxy = read("proxy.ts"), layout = read("app/layout.tsx"), css = read("app/globals.css"), shell = read("components/layout/RouteShell.tsx"), manager = read("components/dashboard/ManagerToday.tsx"), cockpit = read("components/dashboard/DirectorCockpit.tsx");
...
for (const metric of ["proposalsSent", "measurementsScheduled", "overdueNextActions", "leadsWithoutNextAction", "pendingPriceApprovals", "sourcePerformance"]) assert(cockpit.includes(metric), `director metric missing: ${metric}`);
