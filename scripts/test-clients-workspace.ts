...
for (const section of ["Что нужно клиенту", "Расчёт и КП", "История общения", "Следующее действие"]) if (!card.includes(section)) throw new Error(`Missing application UI section: ${section}`);
...
for (const field of ["Имя", "Номер WhatsApp", "Город", "Ответственный менеджер", "Что нужно клиенту"]) if (!modal.includes(field)) throw new Error(`Minimal application field missing: ${field}`);
for (const forbidden of ["Предварительная сумма", "Источник заявки", "Статус"]) if (modal.includes(forbidden)) throw new Error(`Technical field leaked into minimal application form: ${forbidden}`);
if (!managersApi.includes("active: true") || !managersApi.includes("Role.MANAGER")) throw new Error("Responsible manager selector is not restricted to active staff");
for (const material of ["Сосна", "Карагач", "Дуб ламель"]) if (!calculator.includes(material) || !proposalApi.includes(material)) throw new Error(`Three-variant flow is missing ${material}`);
for (const action of ["Сформировать КП", "Отправить в WhatsApp", "КП отправлено", "Оформить заказ", "Новая версия"]) if (!proposalWorkspace.includes(action)) throw new Error(`Proposal action missing: ${action}`);
for (const protectedKey of ["internalCost", "workshopPrice", "managerMinimumPrice", "grossDifference", "companyProfit"]) if (!read("lib/lead-calculation-view.ts").includes(protectedKey)) throw new Error(`Manager redaction misses ${protectedKey}`);
